...
Source reviewed: `scripts/capture_screenshot.py`, `scripts/capture_screenshot.ps1`, `scripts/find_macos_window_id.m`, `tests/test_capture_screenshot.py`, `install.sh`. No code changes landed on `main` since the 2026-07-12 review.

---

### Security

No new findings. The intermediate-component symlink TOCTOU between `_validate_output_root` and `ensure_private_directory` (reported 2026-07-12) and the leading-dash PowerShell parameter-binding injection (reported 2026-07-12) remain the outstanding items. PowerShell's `Capture-ToDestination` also has a narrow window between the `Get-Item` reparse-point check and the subsequent `Move-Item`, but the screenshot directory is locked to the current user via `Protect-Directory` ACLs, so no external principal can plant a reparse point there; this is informational only.

---

### Bugs & regressions

**[low, NEW] `xdotool search --name` performs case-sensitive ERE matching while all other resolution paths use case-insensitive comparison (`capture_screenshot.py`, `resolve_linux_named_window`, line 392)**

`resolve_linux_named_window` passes `_escape_ere(query)` directly to `xdotool search --name`. `xdotool` calls POSIX `regcomp(3)` with `REG_EXTENDED` and no `REG_ICASE` flag, so the match is case-sensitive. In contrast, `_matches()` uses `casefold()` (case-insensitive), the macOS CoreFoundation search uses `kCFCompareCaseInsensitive`, and the Windows `IndexOf` comparison uses `StringComparison::OrdinalIgnoreCase`. A user who queries `"firefox"` on Linux will silently get `no_matching_window` when the window title is `"Firefox"` — the same query that succeeds on macOS and Windows.

_Suggested fix:_ Build a case-insensitive POSIX ERE by converting each alphabetic character in the escaped query to a character class (`f` → `[fF]`, etc.):
```python
def _ere_case_insensitive(value: str) -> str:
    result = []
    for ch in _escape_ere(value):
        if ch.isalpha():
    ...
    return "".join(result)
```
Then use `_ere_case_insensitive(query)` in the `xdotool` invocation. (POSIX ERE via glibc does not support the `(?i)` inline flag, so character-class expansion is the portable approach.)

---

### Data leaks

No new findings. Privacy invariants (window-title never echoed back, labels derived from user queries rather than OS titles) remain intact across all platform paths reviewed.

---

### UX

**[low, NEW] `clang` compilation in `resolve_macos_with_helper` is not silenced: warnings and informational output from the compiler appear on the user's terminal (`capture_screenshot.py`, `resolve_macos_with_helper`, line 339)**

```python
subprocess.run([clang, "-framework", "ApplicationServices", str(helper_source), "-o", str(helper)], check=True)
```

`stdout` and `stderr` are inherited from the parent process. On some macOS configurations clang emits deprecation warnings (e.g. for `CGWindowListCopyWindowInfo`, which Apple has flagged deprecated in macOS 14+). These warnings are printed to the calling terminal interleaved with the tool's structured output (file paths or error codes expected by the agent). If `check=True` raises `CalledProcessError`, the error message contains no compilation detail because stderr was never captured.

_Suggested fix:_ Add `capture_output=True`; on non-zero return, re-raise as a structured `die(f"clang failed: {proc.stderr.strip()}", EXIT_UNAVAILABLE)` so the caller gets a readable error. On success the captured stderr can be discarded to keep console output clean.

**[info, NEW] `plan_capture` accepts a `label: str` parameter that is never used within the function body (`capture_screenshot.py`, `plan_capture`, lines 200–299)**

AST analysis confirms `label` is absent from all name references inside `plan_capture`. The label only affects file naming via `prepare_output_paths` / `unique_capture_path`, not the command tuples. The unused parameter creates a misleading API surface that could prompt a future contributor to wire it into a command argument expecting that it is already sanitized (it is, via `sanitize_label`, but that happens at the call site, not inside `plan_capture`).

_Suggested fix:_ Remove `label` from `plan_capture`'s signature and the corresponding argument at its single call site (line 628). No other change is required.
...
**[medium, carry-over] PowerShell parameter-binding injection via leading-dash `--query` values (`capture_screenshot.py`, `_run_powershell_script`, first reported 2026-07-12)**
Still unresolved.

**[info, carry-over] TOCTOU between `_validate_output_root` and `ensure_private_directory` via intermediate-component symlink replacement (`capture_screenshot.py`, first reported 2026-07-12)**
Still unresolved.
...
**[low, NEW] `unique_capture_path` does not check for dangling symlinks (`capture_screenshot.py:93`)**
`unique_capture_path` uses `candidate.exists()` to decide whether a filename slot is free. `Path.exists()` returns `False` for a dangling symlink (one whose target does not exist), so a dangling symlink at `label.png` is selected as the output path. `execute_plan` then guards with `output.exists() or output.is_symlink()` (line ~496) and rejects the path with "refusing to overwrite an existing screenshot path" — a confusing error because no visible file exists. The inconsistency also means that on any future refactor that removes the `execute_plan` guard, the dangling symlink becomes a write target. Fix: change the check to `candidate.exists() or candidate.is_symlink()` to match `execute_plan`.

**[low, NEW] `Protect-File` applies Windows ACL with no post-ACL verification (`capture_screenshot.ps1:115-128`)**
`Protect-File` calls `Set-Acl` to restrict the screenshot file to the current user and returns immediately with no verification that the ACL was applied. By contrast, Python's `ensure_private_directory` reads back the mode with `path.stat()` and asserts `mode & 0o077 == 0` after `chmod`. On Windows, `Set-Acl` can silently fail in certain edge cases (e.g., a filesystem that does not support DACLs, a Remote Desktop session where the underlying share overrides ACLs). Fix: after `Set-Acl`, re-read the ACL with `Get-Acl` and verify that only the expected identity's rule is present, mirroring Python's post-chmod stat check.

### Bugs & regressions

**[low, NEW] `Copy-Rectangle` result is never checked for all-black pixels (`capture_screenshot.ps1:295-349`)**
`Test-BitmapAllBlack` is called only when `$Handle -ne [IntPtr]::Zero` (the `Copy-Window`/`PrintWindow` path, line 309). The `else` branch at line 314 calls `Copy-Rectangle` (`CopyFromScreen`) for fullscreen and active-without-handle captures but never checks the resulting bitmap. In headless Remote Desktop, Citrix, or Server Core sessions where GPU acceleration is disabled, `CopyFromScreen` completes without throwing but returns a solid-black bitmap. The script saves the black PNG, emits the file path, and exits 0 — no advisory warning. Fix: call `Test-BitmapAllBlack` on the `Copy-Rectangle` result and emit the same `[Console]::Error.WriteLine` advisory that `Copy-Window` emits.

**[info, NEW] `_linux_window_is_viewable` xprop-failure-with-xwininfo-fallback path is not tested (`tests/test_capture_screenshot.py`)**
Existing tests cover: (a) xprop present and returns "Iconic" → not viewable; (b) xprop present and returns "Normal" → viewable; (c) only xwininfo present → viewable; (d) neither tool present → IDs kept. No test covers xprop present but exiting with a non-zero status code, which should cause fallthrough to the xwininfo branch. Fix: add a test with a fake xprop that returns exit 1 and a fake xwininfo that returns "IsViewable", asserting the result is viewable (`result.ok == True`).
...
**[low, NEW] `_run_powershell_script` subprocess call has no timeout (`capture_screenshot.py:546`)**
`subprocess.run(cmd)` at line 546 omits a `timeout` parameter, so a hung PowerShell invocation blocks the Python process indefinitely. Prior entries (2026-06-11 for `run_command`/`clang`, 2026-07-05 for `copy_file_to_clipboard`) covered adjacent call sites; this delegation call to the PowerShell script was not previously documented. A hung PowerShell invocation can arise from GDI exhaustion, a Windows permission dialog waiting for a button click, or MTA clipboard operations that stall. Fix: pass `timeout=120` and catch `subprocess.TimeoutExpired` with a user-readable error message.

### Carry-overs from 2026-07-14

**[medium, carry-over] `ensure_private_directory` creates the directory before confirming no symlink occupies the path (`capture_screenshot.py`, `ensure_private_directory`, first reported 2026-06-08)**
Still unresolved.

**[medium, carry-over] `screencapture -l` window-ID integer overflow: CGWindowID is `uint32_t` but stored as signed `int` in the helper; IDs > 2 147 483 647 wrap negative (`find_macos_window_id.m:86`, first reported 2026-06-08)**
Still unresolved.

**[medium, carry-over] `plan_capture` (Linux/X11 window) calls `shlex.quote` on `window_ids` but `xdotool search` already has the ERE query injected unquoted via `_escape_ere`, and a crafted window name could still affect downstream tools (`capture_screenshot.py`, first reported 2026-06-10)**
Still unresolved.
...
**[low, carry-over] `copy_file_to_clipboard` reads the entire PNG into memory with no size cap before passing it to `pbcopy`/`xclip`/`xsel` subprocess (`capture_screenshot.py`, first reported 2026-07-05)**
Still unresolved.

**[low, carry-over] `copy_file_to_clipboard` subprocess call has no timeout; a stalled clipboard daemon blocks the process indefinitely (`capture_screenshot.py`, first reported 2026-07-05)**
Still unresolved.

**[low, carry-over] `_run_powershell_script`: `powershell.exe` is resolved from `$PATH` without checking that the resolved binary is inside a trusted system directory, allowing a rogue `powershell.exe` earlier in `$PATH` to be executed (`capture_screenshot.py`, first reported 2026-07-12)**
Still unresolved.

**[low, carry-over] `resolve_macos_with_helper`: compiled helper binary written to a world-accessible temp directory; concurrent attacker could replace the binary between write and exec (`capture_screenshot.py`, first reported 2026-06-08)**
Still unresolved.
...
**[low, carry-over] `install.sh` does not verify that the cloned files are owned by the current user before adding them to the skill directories, allowing a race between clone and install on multi-user systems (first reported 2026-06-20)**
Still unresolved.
...
**[info, NEW] `_validate_integer_ids` uses `str.isdigit()`, which accepts Unicode decimal-digit characters (`capture_screenshot.py:83–86`)**
`str.isdigit()` in Python returns `True` for Unicode decimal-digit code points such as `²` (U+00B2), `³` (U+00B3), and fullwidth digits (`１`, `２`, …). A window-lookup helper that produced any of these in its output would pass `_validate_integer_ids` and the resulting string would then be passed as a window-ID argument to `screencapture -l` or `xdotool`, which would reject it (or misinterpret it), causing an opaque tool error rather than a clear validation failure. All current helpers (`xdotool` and the compiled C binary that uses `printf("%d\n", …)`) produce only ASCII decimal digits, so this is theoretical. However, the intent of the validation is clearly "only ASCII decimal digits".
_Suggested fix:_ Replace `id_str.isdigit()` with `id_str.isascii() and id_str.isdigit()`, or `all(c in '0123456789' for c in id_str) and id_str`, to restrict acceptance to ASCII digits 0–9.

---

### Bugs & regressions

**[medium, NEW] PowerShell capture script is not DPI-process-aware; screenshots are low-resolution on high-DPI displays with no warning (`capture_screenshot.ps1:224–246`)**
The PowerShell process runs at the system default DPI awareness level (DPI-unaware / system DPI-unaware). Both `GetWindowRect` (via `Win32Capture.GetWindowRect`) and `Graphics.CopyFromScreen` operate in logical-pixel (96 DPI) coordinates. On displays with fractional scaling (125 %, 150 %, 200 %+), the captured bitmap contains `logical_width × logical_height` pixels — a fraction of the physical-pixel count. For example, on a 3840×2160 display at 200 % scaling, the fullscreen capture produces a 1920×1080 PNG without any advisory message. `PrintWindow` with `PW_RENDERFULLCONTENT` (0x2) renders the window at its logical size as reported by the DPI-unaware process, similarly capping resolution. No warning is emitted to the user.
_Suggested fix:_ Call `SetProcessDpiAwareness(PROCESS_PER_MONITOR_DPI_AWARE)` (via `Add-Type` / P/Invoke to `shcore.dll`) at script startup, or call `SetProcessDPIAware()` (user32.dll) as a minimum. After doing so, also acquire physical-pixel bounds via `DwmGetWindowAttribute(DWMWA_EXTENDED_FRAME_BOUNDS)` for per-window captures to avoid DWM shadow padding errors.

**[low, NEW] PowerShell `Get-WindowBounds` throws an unstructured terminating error when a visible, non-minimized window reports zero-area bounds (`capture_screenshot.ps1:226–234`)**
```powershell
if ($width -le 0 -or $height -le 0) {
    throw 'window has no drawable bounds'
}
```
This `throw` statement, combined with `$ErrorActionPreference = 'Stop'`, exits the entire script with a PowerShell error record on stderr and exit code 1. Exit code 1 is not one of the documented exit codes used by the Python orchestrator (64/73/74/75). Visible, non-minimized system windows (tool windows, off-screen windows that have not been clipped, windows that report `IsWindowVisible()==true` but are zero-size) can trigger this path. The Python caller would see exit code 1 and propagate it via `subprocess.run(cmd).returncode` without a user-readable structured message.
_Suggested fix:_ Replace the `throw` with `[Console]::Error.WriteLine("window_not_capturable: '$Label' has no drawable area — it may be off-screen or a zero-size system window."); exit 75` to use the documented `EXIT_NOT_CAPTURABLE` exit code and a structured message consistent with the rest of the script.
...
**[info, NEW] `New-TemporaryCapturePath` names temp files with a leading dot, inconsistent with Python's `private_temp_png` (`capture_screenshot.ps1:163`)**
```powershell
$candidate = Join-Path $Folder ('.{0}.{1}.{2:D3}.tmp.png' -f $stem, $PID, $i)
```
The Windows temp filename begins with a period. On Windows, a leading dot confers no special file-system semantics (hidden files require the `Hidden` attribute), so there is no practical impact today. By contrast, the Python implementation at `scripts/capture_screenshot.py:441–448` explicitly avoids a leading dot (`{final_path.stem}.{os.getpid()}.{index:03d}.tmp.png`) because macOS `screencapture` refuses to write to dot-prefixed paths — and this constraint is documented in both a code comment and a unit test. A future contributor porting the temp-file logic cross-platform (or adding a macOS PowerShell path) might not notice the asymmetry.
_Suggested fix:_ Remove the leading dot: `('{0}.{1}.{2:D3}.tmp.png' -f $stem, $PID, $i)` to keep the naming consistent with Python and future-proof against any macOS support.

---

### Carry-overs from 2026-07-15
...
Still unresolved. `candidate.exists()` returns `False` for a dangling symlink, so the slot is considered free and the dangling symlink path is returned; `execute_plan` then catches it with the less-clear "refusing to overwrite" guard.
...
**[high, carry-over] Linux X11 named-window clipboard capture crashes with "internal error: missing output path" (`capture_screenshot.py`, first reported 2026-06-10)**
Still unresolved. Now 35 days old.
...
**[info, NEW-MISSING carry-over] `Find-WindowHandles` assigns to `$matches`, shadowing the PowerShell automatic variable `$Matches` (`capture_screenshot.ps1:204`, first reported 2026-07-14)**
Still unresolved. (This finding was listed as [info, NEW] in the 2026-07-14 UX section but omitted from the 2026-07-15 carry-over list; included here for completeness.)
...
**[medium, NEW] `_validate_output_root` accepts paths that resolve precisely to `$HOME` itself, causing `ensure_private_directory` to chmod the user's home directory (`capture_screenshot.py:522–528`)**
`path.resolve().relative_to(home)` succeeds when `path.resolve() == home` (returns `Path('.')` without raising `ValueError`). A caller passing `--output-root ~/foo/..` or any other path whose resolved form equals `$HOME` passes validation, then `ensure_private_directory` receives the original unresolved path. The OS resolves `..` implicitly, so `path.mkdir(parents=True, exist_ok=True)` is a no-op on `$HOME` (already exists), then `path.chmod(0o700)` sets the home-directory mode to `0o700`. This silently tightens the user's home-directory permissions (which may break group-accessible setups, setgid scripts, or web-hosting arrangements), and does so without any warning or explanation. The path need not include `..` literally; any symlink chain that resolves to `$HOME` produces the same effect, though the `ensure_private_directory` symlink check would catch that case.
_Suggested fix:_ In `_validate_output_root`, after resolving, assert `resolved != home` in addition to the `relative_to` check: `if path.resolve() == home or ...: die(...)`. Alternatively, normalize `args.output_root` to its resolved form in `parse_args` so that `ensure_private_directory` always operates on a canonical path.

---

### Bugs & regressions

**[low, NEW] PowerShell `New-CapturePath` in dry-run mode does not use a `reserved`-path set; multiple windows with the same label receive duplicate filenames in dry-run output (`capture_screenshot.ps1:143–157`)**
In dry-run mode, `$script:RequestFolder` is set but not created on disk. `New-CapturePath` checks `Test-Path -LiteralPath $candidate`, which returns `$false` for every candidate inside a non-existent folder, so every call for the same label independently concludes that `{label}.png` is available. When `--allow-multiple-matches` surfaces two or more windows with the same query string (and therefore the same sanitized label), all of them report the same output filename (e.g. `chrome.png`, `chrome.png`) in dry-run. The actual run would produce `chrome.png` and `chrome-001.png`. The Python side avoids this with a `reserved: set[Path]` passed through `prepare_output_paths` and `unique_capture_path` (lines 429–434 of `capture_screenshot.py`); the PowerShell side has no equivalent.
_Suggested fix:_ Track allocated dry-run paths in a script-scoped `HashSet[string]` and pass it into `New-CapturePath`, checking both `Test-Path` and the set before returning a candidate path.
...
**[high, carry-over] Linux X11 named-window clipboard capture crashes with "internal error: missing output path" (`capture_screenshot.py`, first reported 2026-06-10)**
Still unresolved. Now 37 days old.
